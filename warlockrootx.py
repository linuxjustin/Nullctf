...
async def help(ctx, page=None):
    if (not page) or (page == '1'):
        page = '1'
        emb = discord.Embed(description=help_page_1, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')
    
    if page == 'ctf':
        emb = discord.Embed(description=help_page_ctf, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')
    
    if page == 'hash':
        emb = discord.Embed(description=help_page_hash, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')

    if page == 'encode':
        emb = discord.Embed(description=help_page_encode, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')
    
    if page == 'rsa':
        emb = discord.Embed(description=help_page_rsa, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')

    if page == 'end':
        emb = discord.Embed(description=help_page_end, colour=4387968)
        emb.set_author(name='<request "x" - request a feature')
...
async def request(ctx, feature):
    creator = await bot.fetch_user(549240760743100436)
